...
export default function OrderPage() {
  ...
  if (state.isLoadingConfig) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <MenuDisplay menuImageUrl="" restaurantName="" isLoading={true} />
      </div>
    );
  }
  ...
  if (state.configError) {
    // 判斷是否為「尚未開放點餐」的訊息
    const isNotOpenYet = state.configError.includes('尚未開放點餐');
      ...
      <div className="min-h-screen bg-gradient-to-br from-stone-50 via-amber-50 to-stone-100 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 border border-gray-200">
          <div className="flex flex-col items-center space-y-4">
            <div className={isNotOpenYet ? 'text-amber-500' : 'text-red-600'}>
              <svg
                className="w-16 h-16"
                fill="none"
                stroke="currentColor"
            ...
            <h2 className="text-xl font-semibold text-gray-900">
              {isNotOpenYet ? '尚未開放點餐' : '載入失敗'}
            </h2>
            <p className="text-gray-600 text-center text-sm">
              {isNotOpenYet ? '目前沒有開放的餐廳，請稍後再試' : state.configError}
            </p>
            <button
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-amber-50 to-stone-100 p-4 sm:p-6 lg:p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {/* 學生姓名輸入 */}
        <StudentNameInput value={state.studentName} onChange={setStudentName} />

        {/* 總金額顯示 */}
        <TotalAmount 
          amount={totalAmount}
          mealSubtotal={state.selectedMeal ? state.selectedMeal.price * state.mealQuantity : 0}
          addonsTotal={state.selectedAddons.reduce((sum, addon) => sum + addon.price, 0)}
        />
